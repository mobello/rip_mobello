...
(function() {
  ...
  if (typeof tau.fx == 'undefined') {
    ...
    /**
     * @name tau.fx
     * @namespace Mobello의 Visual effects들을 정의한다.
     */
    ...
    tau.fx = {
      ...
      /**
       * element의 현재 opacity를 점차 0으로 만들어 사라지게 하는 애니메이션 
       * @example
       * <pre>
       * <code>
       ...
       * </code>
       * </pre>
       * @param {Element} elem DOM의 element, 만약 component일 경우 getDOM함수를 사용해야함
       * @param {Object} [opts] 애니메이션의 옵션
       * @param {String | Function} [opts.timingFunction] 변화해가는 속도 변화 (ease-in,ease-out,ease-in-out,defaultease,cubic-bezier)
       * @param {Number | String} [opts.duration] 애니메이션이 진행되는 지속 시간(ms) ,String도 가능(ex: '10s')
       * @param {Number} [opts.iterationCount] 반복횟수
       * @param {Number | String} [opts.delay] 애니메이션 지연 시간(ms) ,String도 가능(ex: '10s')
       * @param {Boolean} [opts.override=true] 애니메이션 변경 유지 여부
       */
      ...
      /**
       * element의 현재 opacity를 점차 100으로 만들어 나타나게 하는 애니메이션
       * @example
       * <pre>
       * <code>
       * tau.fx.fadeOut(button1.getDOM(),{duration : 500}); 
       * </code>
       * </pre>
       * @param {Element} elem DOM의 element, 만약 component일 경우 getDOM함수를 사용해야함
       * @param {Object} [opts] 애니메이션의 옵션
  ...
  //------------------------------------------------------------------------------
    /** @lends tau.fx.Transition.prototype */
    $class('tau.fx.Transition').define({
      $static: /** @lends tau.fx.Transition */
      /** @static */
      {
        /** default options */
        DEFAULT_OPTIONS: {
      ...
      /**
       * 생성자, 새로운 객체를 생성한다.
       * @class 현재 style에서 다른 style로 순차적 전환하여 애니메이션 효과를 나타나게 하는 
       * 클래스이다. 애니메이션은 animate함수를 호출해야 시작된다.
       * 
       * <pre>
       * <code>
       ...
       * 
       * @constructs
       * @param {Object} opts 디폴트 애니메이션 옵션
       * @param {String | Function} [opts.timingFunction] 변화해가는 속도 변화 (ease-in,ease-out,ease-in-out,defaultease,cubic-bezier)
       * @param {Number | String} [opts.duration] 애니메이션이 진행되는 지속 시간(ms) ,String도 가능(ex: '10s')
       * @param {Number | String} [opts.delay] 애니메이션 지연 시간(ms) ,String도 가능(ex: '10s')
       */
      Transition: function(opts) {
        ...
        this._style = {};
        this._options =  tau.mixin(opts, tau.fx.Transition.DEFAULT_OPTIONS);
      },
      
      /**
       * style속성값으로 Transition 애니메이션을 설정한다.
       * @param {String} name style 속성
       * @param {String} value 속성값
       * @param {Object} opts 속성에 대한 개별 애니메이션 옵션
       * @param {String | Function} [opts.timingFunction] 변화해가는 속도 변화 (ease-in,ease-out,ease-in-out,defaultease,cubic-bezier)
       * @param {Number | String} [opts.duration] 애니메이션이 진행되는 지속 시간(ms) ,String도 가능(ex: '10s')
       * @param {Number | String} [opts.delay] 애니메이션 지연 시간(ms) ,String도 가능(ex: '10s')
       */
      setStyle: function(name, value, opts) {
        this._style[name] = {
          value: value||"",
          options: opts||{}
        };
...
//        this._target.style["-moz-transition"] = tArray.toString();
      ...
      /**
       * Transition 애니메이션을 실행하는 함수
       * @param {Element} target 애니메이션 타겟 DOM의 element, 만약 component일 경우 getDOM함수를 사용해야함
       * @param {Object} animOpts 실행에서의 개별 애니메이션 옵션
       * @param {String | Function} [opts.timingFunction] 변화해가는 속도 변화 (ease-in,ease-out,ease-in-out,defaultease,cubic-bezier)
       * @param {Number | String} [opts.duration] 애니메이션이 진행되는 지속 시간(ms) ,String도 가능(ex: '10s')
       * @param {Number | String} [opts.delay] 애니메이션 지연 시간(ms) ,String도 가능(ex: '10s')
       */
      ...
      /**
       * animation이 끝날 때 호출하는 이벤트 핸들러가 있으면 호출 하고 이벤트 핸들러를 삭제하는 함수
      ...
      /**
       * Transition이 완료된 후 _run에서 등록한 tau.rt.Event.TRANSITIONEND 이벤트 핸들러 함수를 호출 한다.
       * @param {Event} e 이벤트 객체
       * @private 
       */
      handleEvent: function(e) {
...
//        this._fireEvent("animationtween");
      ...
      /**
       * javascript animation을 실행시키는 함수
       * @private
       */
      run: function() {
        this._initAnimAttrs();
...
//        tau.log("run(JsAnimUnit):"+this.tid);
  ...
  //------------------------------------------------------------------------------
    /** @lends tau.fx.Animation.prototype */
    $class('tau.fx.Animation').define({
      ...
      /**
       * 생성자, 새로운 Animation객체를 생성한다.
       * @class 구간별로 style을 지정하여 애니메이션 효과를 나타나게 하는 클래스이다. 
       * <pre>
       * <code>
       ...
       * </pre>
       * 
       * @param {Object} [keyframes] 애니메이션의 키프레임, 현재 from to 만 지원함
       * @param {Object} [keyframes.from] 시작할 때의 styles 객체
       * @param {Object} [keyframes.to] 끝날때의 styles 객체
       * @param {Object} [opts] 애니메이션의 디폴트 옵션
       * @param {String | Function} [opts.timingFunction] 변화해가는 속도 변화 (ease-in,ease-out,ease-in-out,defaultease,cubic-bezier)
       * @param {Number | String} [opts.duration] 애니메이션이 진행되는 지속 시간(ms) ,String도 가능(ex: '10s')
       * @param {Number} [opts.iterationCount] 반복횟수
       * @param {Number | String} [opts.delay] 애니메이션 지연 시간(ms) ,String도 가능(ex: '10s')
       ...
       * 
       * @param {Element} target 애니메이션 타겟 DOM의 element, 만약 component일 경우 getDOM함수를 사용해야함
       * @param {Object} [opts] 실행에서의 개별 애니메이션 옵션
       * @param {String | Function} [opts.timingFunction] 변화해가는 속도 변화 (ease-in,ease-out,ease-in-out,defaultease,cubic-bezier)
       * @param {Number | String} [opts.duration] 애니메이션이 진행되는 지속 시간(ms) ,String도 가능(ex: '10s')
       * @param {Number} [opts.iterationCount] 반복횟수
       * @param {Number | String} [opts.delay] 애니메이션 지연 시간(ms) ,String도 가능(ex: '10s')
