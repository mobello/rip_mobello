...
tau.ui.Component.prototype.renderer = tau.mixin({
  ...
  updateDisabled: function ($renderData, disabled) {
    var $dom = $renderData.$dom,
      $base = $renderData.$base,
      root = $dom[tau.ui.ROOT_KEY];
    
    tau.util.dom[disabled ? 'addClass' : 'removeClass'](root, '-disabled', $base);
  }
}, tau.ui.BaseRenderer);
...
tau.ui.Button.prototype.renderer = tau.mixin({
   ...
   *  </ul> 
   */
  updateState: function ($renderData, state) {
    var $dom = $renderData.$dom,
        $base = $renderData.$base,
        root = $dom[tau.ui.ROOT_KEY];
    switch (state) {
    case 'selected':
      tau.util.dom.replaceClass(root, '-highlighted', '-selected', $base);
      break;
    case 'highlighted':
...
tau.ui.TextField.prototype.renderer = tau.mixin({
  ...
  updateControl: function ($renderData, state) {
    var $dom = $renderData.$dom;
    $dom[tau.ui.TextField.TEXT_KEY].disabled = (state === 'disabled');
  },
  ...
  focus: function ($renderData) {
    var $dom = $renderData.$dom;
    $dom[tau.ui.TextField.TEXT_KEY].focus();
    tau.util.dom.addClass($dom[tau.ui.ROOT_KEY], '-highlighted', $renderData.$base);
    tau.log('focus....');
  },
  ...
  blur: function ($renderData) {
    var $dom = $renderData.$dom;  
    $dom[tau.ui.TextField.TEXT_KEY].blur();
    tau.util.dom.removeClass($dom[tau.ui.ROOT_KEY], '-highlighted', $renderData.$base);
    tau.log('blur....');
  }
...
tau.ui.ScrollPanel.prototype.renderer = tau.mixin({
  ...
  updatePullToRefresh: function ($renderData, pullToRefresh, vertical, type, text) {
    ...
    case tau.ui.ScrollPanel.PULL_PULLTOREFREH:
      ...
      this.initPullUpToRefresh($renderData, vertical);
      break;
...
tau.ui.Switch.prototype.renderer = tau.mixin({
  ...
  _initializeDOM: function ($dom, $base, root) {
    var container = root.firstChild; 
    $dom[tau.ui.CONTENT_KEY] = container;
    $dom[tau.ui.Switch.BEFOREBAR_KEY] = container.firstChild;
    $dom[tau.ui.Switch.THUMB_KEY] = container.childNodes[1];
    $dom[tau.ui.Switch.AFTERBAR_KEY] = container.childNodes[2];
  },
  ...
  updateOnText: function ($renderData, text) {
    var $dom = $renderData.$dom;
    $dom[tau.ui.Switch.BEFOREBAR_KEY].innerHTML = text;
  },
  ...
  updateOffText: function ($renderData, text) {
    var $dom = $renderData.$dom;
    $dom[tau.ui.Switch.AFTERBAR_KEY].innerHTML = text;
  },
  ...
  updateThumb: function ($renderData, vertical, position){
    var $dom = $renderData.$dom,
      thumb = $dom[tau.ui.Switch.THUMB_KEY];
    
    tau.util.style($dom[tau.ui.Switch.AFTERBAR_KEY], vertical ? 'top' : 'left', 
        position + 'px');

    if(vertical){
      tau.util.style($dom[tau.ui.Switch.BEFOREBAR_KEY], 'height', 
          (thumb.offsetHeight + position) + 'px');
      tau.util.style(thumb, 'top', position + 'px');
    } else {
      tau.util.style($dom[tau.ui.Switch.BEFOREBAR_KEY], 'width', 
          (thumb.offsetWidth + position) + 'px');
      tau.util.style(thumb, 'left', position + 'px');
    }
  },
  ...
  getThumbPosition: function ($renderData, vertical, touch){
    ...
    if (touch){
      ...
      return vertical ? 
          $dom[tau.ui.Switch.THUMB_KEY].offsetTop :
          $dom[tau.ui.Switch.THUMB_KEY].offsetLeft;
    }
    return 0;
  },  
  ...
  getRange: function ($renderData, vertical){
    ...
    if(vertical){
      range = $dom[tau.ui.CONTENT_KEY].offsetHeight - 
      $dom[tau.ui.Switch.THUMB_KEY].offsetHeight;
    } else {
      range = $dom[tau.ui.CONTENT_KEY].offsetWidth - 
      $dom[tau.ui.Switch.THUMB_KEY].offsetWidth;
    }
    return range;
  }  
...
tau.ui.TextArea.prototype.renderer = tau.mixin({
  ...
  _templateControl: ['<textarea class=${base}${control}></textarea>'],
   ...
   */
  _initializeDOM: function ($dom, $base, root) {
    tau.ui.TextArea.$super.renderer._initializeDOM.apply(this, arguments);
...
//    }
  ...
  focus: function ($renderData) {
    var $dom = $renderData.$dom;
    tau.util.dom.addClass($dom[tau.ui.ROOT_KEY], '-highlighted', $renderData.$base);
    $dom[tau.ui.TextArea.CONTROL_KEY].focus();
  },
  ...
  blur: function ($renderData) {
    var $dom = $renderData.$dom;
    tau.util.dom.removeClass($dom[tau.ui.ROOT_KEY], '-highlighted', $renderData.$base);
    $dom[tau.ui.TextArea.CONTROL_KEY].blur();
  },
...
tau.ui.Checkbox.prototype.renderer = tau.mixin({
   ...
   */
  updateState: function ($renderData, checked) {
    var $dom = $renderData.$dom,
      $base = $renderData.$base,
      root = $dom[tau.ui.ROOT_KEY];
    
    tau.util.dom[checked ? 'addClass' : 'removeClass'](root, '-selected', $base);
  }
...
tau.ui.ActionSheet.prototype.renderer = tau.mixin({
  ...
  $styleClass: tau.mixin({
    ...
    large: '-large' ,
    title: '-title'
  }, tau.ui.ActionSheet.$super.renderer.$styleClass),
  ...
  open: function ($renderData, opts) {
    ...
    tau.util.dom.addClass(popup, 'tau-preserve3d');
