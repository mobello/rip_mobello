 ...
 * 
 *  @example <h3>기본 스타일 클래스 적용하기</h3>
 *  var button = new tau.ui.Button({
 *    label: 'BUTTON'
 *    baseStyleClass: 'dark'
 *  });
 *  
 *  @example <h3>CSS 스타일 적용하기</h3>
 *  var button = new tau.ui.Button({
 *    styles: {
 *      fontSize: '1.2em',
 *      color: 'red'
...
$class('tau.ui.Drawable').define({
  ...
  /**
   * 현재 인스턴스의 DOM 앨리먼트의 ID를 반환한다. 
   * 만약 현재 생성된 DOM 앨리먼트의 ID가 존재하지 않을 경우 새로운 ID를 생성해서 반환하며, 
   * 이때 생성된 ID는 현재 인스턴스의 DOM 앨리먼트 ID가 된다.
   * <p />
   ...
   * tau.log.info('systemId: '+ button.getId(true)); // systemId: button1__tau1339480292548 
   * 
   ...
   * @returns {Boolean} 이전에 {@link tau.ui.Drawable#draw}가 호출되어 화면에
