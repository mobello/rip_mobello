...
(function(global) {
  ...
  /** @lends tau.data.XMLRequest.prototype */
  $class('tau.data.XMLRequest').define({
    /**
     * 명시된 옵션들을 이용하여 새로운 객체를 생성한다.
     * Request 처리에 따라 다음과 같은 시스템 이벤트가
     * 발생된다.<p/>
     * {@link tau.rt.Event.REQ_SENT}: Request가 전송되었을
     * 때<br/>
     * {@link tau.rt.Event.REQ_RECEIVED}: Response를 받았을
     * 때<br/>
     * {@link tau.rt.Event.REQ_ABORTED}: Request가 중지되었을
     * 때<p/>
     * 시스템 이벤트를 받기 위해서는 다음과 같이 Runtime에
     * 이벤트 리스너를 달아야 한다.
     * @example
     * var rt = tau.getRuntime();
     * rt.onEvent(tau.rt.Event.REQ_SENT, function (e, payload) {
     *   ...
     * });
     *
     * @class
     ...
     * @constructs
     * @param {Object} [opts] XMLHttpRequest 요청에 필요한
     * 옵션들 
     * @param {String} [opts.type = 'GET'] Request 방식(GET,
     * POST중 택일)
     * @param {String} [opts.contentType = 'application/x-www-form-urlencoded'] 콘텐츠 인코딩 방식
     * @param {String} [opts.encoding = 'UTF-8'] 인코딩 방식
     * @param {Boolean} [opts.async = true] Sync or
     * Async전송방식 결정
     * @param {Number} [opts.timeout = 5000] Request 최대
     * Timeout시간 (milliseconds)
     * @param {Boolean} [opts.silent = false] 시스템 이벤트
     * 발생 여부
     * @param {String} [opts.url] Request를 전송하기 위한 URL
     * @see tau.data.XSSRequest
     */
    ...
    /**
     * XMLHttpRequest를 이용하여 Request를 전송한다.
     * @param {String} url Request를 전송할 URL
     * @param {Function} callbackFn Request전송후 응답(success/fail)이
     * 왔을 때의 callback함수
     * @param {Object} opts 생성자에서 정의한 옵션들을
     * Override할 수 있다.
    ...
    /**
     * 명시된 id를 이용하여 이전에 전송했던 Request를 중지시킨다. 내부적으로
     * {@link tau.rt.Event.REQ_ABORTED}이벤트가 발생된다.
    ...
    /**
     * XMLHttpRequest는 Garbage Collection을 수행하지 않는다.(empty function)
     * @function
     */
    gc: tau.emptyFn
  });
  ...
  /** @lends tau.data.XSSRequest.prototype */
  $class('tau.data.XSSRequest').define({
    /**
     * 명시된 옵션을 이용하여 새로운 객체를 생성한다.
     * Request 처리에 따라 다음과 같은 시스템 이벤트가
     * 발생된다.<p/>
     * {@link tau.rt.Event.REQ_SENT}: Request가 전송되었을
     * 때<br/>
     ...
     * @constructs
     * @param {Object} [opts] XSSRequest 요청에 필요한
     * 옵션들 
     * @param {String} [opts.type = 'JSONP'] Request 방식
     * @param {String} [opts.contentType = 'application/json'] 콘텐츠 인코딩 방식
     * @param {String} [opts.encoding = 'UTF-8'] 인코딩 방식
     * @param {Number} [opts.timeout = 5000] Request 최대
     * Timeout시간 (milliseconds)
     * @param {Boolean} [opts.silent = false] 시스템 이벤트
     * 발생 여부
     * @param {Boolean} [opts.jsonpCallback = 'callback']
     * Callback함수를 지정하기 위한 키
     * @param {String} [opts.url] Request를 전송하기 위한 URL
     * @see tau.data.XMLRequest
     */
    ...
    /**
     * 동적 스크립트 로딩 방식으로 Requet를 전송한다.
     * XSSRequest는 항상 Async 방식으로 통신한다.
     * @param {String} url Request를 전송할 URL
     * @param {Function} callbackFn Request전송후 응답(success/fail)이
     * 왔을 때의 callback함수
     * @param {Object} opts 생성자에서 정의한 옵션들을
    ...
    /**
     * 처리되지 않은 Request들을 Garbage collect한다.
     * @param {Number} buff 추가적인 timeout
     * buffer(milliseconds)
     */
    gc: function (buff) {
